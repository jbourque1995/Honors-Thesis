...
import sys
sys.path.append('/content/drive/MyDrive/Honors_Thesis')
    
from autoregressive_diffusion.experiments.images import config as config_module
...
from save_generated_images_for_discriminator import save_generated_images_for_discriminator

writer = metric_writers.create_default_writer(
    logdir="/content/drive/MyDrive/Honors_Thesis/Output/tensorboard_logs"
)

def train_step(rng, batch, state, disc_bonus, model, config):
    def loss_fn(params):
        ...
        loss += config.disc_weight * disc_bonus
    ...
    if config.clip_grad > 0:
        grads, grad_norm = util_fns.clip_by_global_norm(grads, clip_norm=config.clip_grad)
    else:
...
def train_epoch(p_train_step, state, train_ds, batch_size, epoch, rng, kl_tracker, config):
    ...
    with jax.profiler.StepTraceAnnotation('train', step_num=state.step):
        for batchnum, batch in enumerate(train_ds):
            ...
            disc_bonus = jnp.array(0.0, dtype=jnp.float32)
            try:
                # Extract and preprocess the image correctly
                img_tensor = batch['image'][0,0] # From shape (1,8,32,32,3)
                single_image = jax.device_get(img_tensor)

                # Ensure it's in uint8 format for PIL
                if single_image.dtype != np.uint8:
                  single_image = np.clip(single_image, 0, 255).astype(np.uint8)

                # Save the image as PNG every 50 pictures
                if batchnum % config.save_interval ==0:
                  folder_path = "/content/drive/MyDrive/Honors_Thesis/tmp_images"
                  os.makedirs(folder_path, exist_ok=True)
                  img_path = os.path.join(folder_path, "sample.png")
                  Image.fromarray(single_image).save(img_path)

                result = subprocess.run(
                    ["python3", "/content/drive/MyDrive/Honors_Thesis/discriminator_bridge.py", folder_path],
                    capture_output=True, text=True
                )
                
                if result.returncode == 0:
                    disc_score = float(result.stdout.strip())
                    disc_bonus = jnp.array(disc_score, dtype=jnp.float32)
                    disc_scores.append(disc_score)
                else:
                    logging.warning(f"Discriminator bridge failed: {result.stderr}")
            except Exception as e:
                logging.warning(f"Discriminator skipped due to error: {e}")

            state, metrics, rng = p_train_step(rng, batch, state, disc_bonus)
...
def eval_model(p_eval_step, rng, state, test_ds, epoch):
  ...
  for i, batch in enumerate(itertools.islice(test_ds, config.max_eval_batches)):
  ...
  epoch_metrics_np['disc_score'] = 0.0

  nelbo = epoch_metrics_np['nelbo']
  logging.info(f'eval epoch: {epoch}, nelbo: {nelbo:.4f}')
...
def log_standard_metrics(writer, train_metrics, eval_metrics, epoch):
  print("Logging Standard Metrics Debug", flush=True)
  metric_dict = {
  ...
  if 'disc_score' in train_metrics:
      metric_dict['disc_score'] = train_metrics['disc_score']
...
def extensive_eval(config, test_rng, writer,
  ...
  if model.num_steps < max_num_steps:
    ...
    logging.info(msg)
    from save_generated_images_for_discriminator import save_generated_images_for_discriminator
    save_generated_images_for_discriminator(chain[-1], epoch)
...
def train_and_evaluate(config,
  ...
  p_train_step = jax.pmap(
    ...
    axis_name='batch',
    in_axes=(None, 0, 0, None),  # rng, batch, state, disc_bonus
    out_axes=(0, 0, None),
    donate_argnums=(2,))
  ...
  print("Starting training at epoch {start_epoch} / {config.num_epochs}", flush = True)
...
if __name__ == "__main__":
    ...
    final_state = train_and_evaluate(config, work_dir="/content/drive/MyDrive/Honors_Thesis/Output/tensorboard_logs")

    from autoregressive_diffusion.experiments.images import ardm_eval
    print("Generating final samples using ARDM_EVAL...", flush=True)
    ardm_eval.evaluate_model(config, final_state)