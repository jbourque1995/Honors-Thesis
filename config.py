...
def get_config():
  ...
  config.batch_size = 8        # was 64
  config.test_batch_size = 200  # Divisible by 8 (TPU pods), divides test 10000
  config.num_epochs = 3        # was 50, was 6000
  config.dataset = 'cifar10'
  config.data_augmentation = False
  config.detailed_eval_every = 3
  config.num_eval_passes = 1
  config.seed = 0
  config.num_samples = 16
  config.use_discriminator = True           
  config.disc_weight = 0.1                  # How strongly discriminator affects training loss
